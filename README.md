...
## Running Qdrant and the models

The plugin needs three reachable endpoints and nothing else: a vector database (Qdrant),
an embedding server, and a rerank server. If you do not already have them somewhere, the
fastest local stack is [llama.cpp](https://github.com/ggml-org/llama.cpp) serving two
[GGUF](https://huggingface.co/gpustack) models. It runs on CPU alone, on x86 and on ARM,
so a MacBook or a laptop can be the whole backend.

The two models are the same ones the search section named, and the same ones the plugin's
defaults configure:

| role | model | GGUF | route |
|---|---|---|---|
| embedding | `bge-m3` (1024-dim) | [gpustack/bge-m3-GGUF](https://huggingface.co/gpustack/bge-m3-GGUF) | `/v1/embeddings` |
| rerank | `bge-reranker-v2-m3` | [gpustack/bge-reranker-v2-m3-GGUF](https://huggingface.co/gpustack/bge-reranker-v2-m3-GGUF) | `/v1/rerank` |
...
The [wizard](#the-one-command) writes the same settings, then re-checks the whole stack,
including the path a process with no shell would read.
...
The fastest path is all three in order: the infrastructure, the host, the wizard. The
infrastructure is the [local stack from the section above](#running-qdrant-and-the-models)
(or your own Qdrant, embedding and rerank servers, wherever they live). Pick the host you
are using and one OS.
...
# 2. the host
claude plugin marketplace add erickstryck/memories-plugin && claude plugin install memories-plugin@memories-plugin
# or: hermes plugins install erickstryck/memories-plugin --enable --force && hermes config set memory.provider memories
...
# or, if the host is hermes:
# bash ~/.hermes/plugins/memories/scripts/install.sh
```

**macOS**

Same three steps; the only lines that change are the infrastructure:
...
Then the host lines and the wizard, exactly as on Linux.
...
llama-server --hf-repo gpustack/bge-m3-GGUF --hf-file bge-m3-Q4_K_M.gguf --embedding --port 8003
llama-server --hf-repo gpustack/bge-reranker-v2-m3-GGUF --hf-file bge-reranker-v2-m3-Q4_K_M.gguf --reranking -c 8192 -b 8192 -ub 8192 --port 8004
# (one terminal per llama-server)
```

Then the host lines, and the wizard in a terminal with `bash`:

```bash
bash ~/.hermes/plugins/memories/scripts/install.sh   # hermes
```

```bash
bash "$(ls -dt ~/.claude/plugins/cache/memories-plugin/memories-plugin/*/scripts/install.sh | head -1)"   # claude-code
```

For Windows there is one extra step that has no OS-neutral shortcut: `~/.local/bin` is not
on PATH by default. Either add it (Settings, system, environment variables) or call the
wizard by its full path as above; the wizard also copies `qctx` there, so once the
directory is on PATH every later step is just `qctx ...`. Without git-bash the host and
wizard lines run under WSL or a Windows build of bash, and the same commands work there
unmodified, with the `~` expanding to that environment's home.

Everything the wizard does in step 3 is expanded under
[The one command](#the-one-command) and in the per-host sections below. On a fully local
stack, answer the two key prompts by pressing Enter.
...
where it says so, and the two steps people skip are 3 and 5, and both fail silently.
