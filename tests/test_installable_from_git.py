...
class TestTheREADMEStatesWhatFAILSSILENTLY(unittest.TestCase):
    """The docs are the only place a new machine gets told what breaks without saying so.

    Three failures on this plugin produce no error at all, a shell-less process with no
    ...
    def test_it_warns_that_config_show_MIXES_file_and_environment(self):
        ...
        self.assertRegex(self.docs(), r"(?i)config show.{0,80}(mix|both)")
