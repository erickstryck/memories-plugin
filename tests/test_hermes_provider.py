...
#: its values can be read back. Every file of BOTH hosts, not just the hermes provider:
#: while this scan covered `hosts/hermes/__init__.py` alone, `hooks/recall.py` kept the one
#: bare `int(env(...))` in the repo for three reviews — the ledger named the blind spot and
#: the scan still could not see into it. A knob is checked because a test reads the FILE it
#: lives in, so the list of files is the coverage.
#:
#: The two file-read guards joined this dict rather than getting a frozen list of their own,
#: so a knob added to either of them tomorrow is covered by tests nobody had to remember to
#: write. Note what the scan reads: `env_num` now LIVES in `core/knobs.py`, shared by both
#: guards, but the CALL SITE stays in each adapter — which is what keeps each guard's knobs
#: derivable from its own file. Move the calls into the shared module and this scan goes
#: blind on both hosts at once; the per-file guard in the first test below is what would
#: report it.
KNOB_SOURCES = {
