...
| Onde nasce o índice | no caminho de **escrita** (`MemoryStore.ensure`) | no caminho de leitura: `require_existing` existe justamente para uma leitura nunca criar nada (`core/memory.py:145`) |
| Paginação | cursor opaco de valor mais ids vistos | `offset` de id: o servidor recusa junto com `order_by` (medição 4) |
| Falha do caminho ordenado | degradar para não ordenado **dizendo que degradou** | falhar a operação inteira; ou degradar em silêncio, que é repetir o defeito de hoje numa forma nova |
...
| ordenado falha mesmo assim | cai para o scroll não ordenado | `"unordered"` mais `warning` no corpo |
...
O `warning` existe para o modelo e para o humano. Uma listagem que caiu para ordem arbitrária
sem avisar é o defeito de hoje com outra roupa.
...
| falha | comportamento |
|---|---|
| coleção não existe | `require_existing` levanta, como hoje. Uma leitura não cria |
| índice não existe | criado no primeiro `ensure`; se a listagem chegar antes de qualquer escrita, criado sob demanda e a operação segue |
| criar índice é proibido (permissão) | `ensure_payload_index` engole a falha; a listagem degrada e **diz** |
| servidor sem suporte a `order_by` | mesma degradação, mesmo aviso |
| cursor corrompido ou de outro acervo | erro claro do `paging`, sem silêncio e sem devolver a página 1 fingindo ser a página 2 |
