...
The [README](../README.md) is the install path. This file is everything behind it:
how the search works, the two hosts and their install gotchas, the big-file read
guard, the layout and the design decisions. Read it when something is not working
the way the README promised, or when you are changing the code.
...
**From zero to working, in order.** Each step is expanded in this file or in the README;
