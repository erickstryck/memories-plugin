"""The invocable surface: one operation table, shared by every host.

WHY THIS IS IN `core/` AND NOT IN A HOST. The schemas, the argument coercion and the
dispatch table are the SAME on every host by construction — `tests/test_host_equivalence.py`
already asserts that what a tool does equals what the matching `qctx` command does, over the
same archive. While the table lived under `hosts/hermes/`, a third adapter had exactly two
options and both were wrong: import a SECOND HOST's package (inheriting its `bind_tuning`
global and its schema prose), or copy ~700 executable lines that the equivalence test only
pins for the two hosts that exist. Here, a new host writes one `dispatch(name, args, cfg=...)`
call.

WHAT STAYS WITH THE HOST: the recall TUNING. `bind_tuning` takes the object whose constants
are this host's floors, because each adapter reads `QCTX_RECALL_*` in its own file — a
requirement of `tests/test_host_equivalence.py`, which derives the knob names from each
adapter's source and requires the two sets to match. A knob read here would read as a setting
the hosts no longer share.

Every handler returns a JSON STRING, and no handler raises. A host that surfaces a raise turns
it into a crashed turn; a JSON error is something the model can read and react to. The string
part is not a detail either: hermes' ABC says "Must return a JSON string", and a dict there
fails at the host boundary, past every test of a handler's own logic.

Each handler routes to the SAME core function the CLI's matching `cmd_*` handler calls, so no
two hosts can drift on what an operation does.
"""
import json
import os
